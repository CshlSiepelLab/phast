...
double tl_compute_log_likelihood(TreeModel *mod, MSA *msa, double *col_scores,
                                 double *tuple_scores, int cat,
                                 TreePosteriors *post) {
  ...
  if (post != NULL) {
    ...
    for (rcat = 0; rcat < mod->nratecats; rcat++) {
      ...
      for (j = 0; j < nstates; j++) {
        ...
        for (k = 0; k < nstates; k++)
          subst_probs[rcat][j][k] =
              (double *)smalloc(mod->tree->nnodes * sizeof(double));
      }
    }
  }
#endif
  ...
  if (!(cat < 0 || col_scores == NULL || msa->categories != NULL))
    die("ERROR tl_compute_log_likelihood: cat=%i, col_scores==NULL=%i, "
        "msa->categories==NULL=%i\n",
  ...
  if (msa->ss != NULL) {
    if (msa->ss->tuple_size <= mod->order)
      die("ERROR tl_compute_log_likelihood: tuple_size (%i) must be greater "
          "than mod->order (%i)\n",
          msa->ss->tuple_size, mod->order);
  } else
    ss_from_msas(msa, mod->order + 1, col_scores == NULL ? 0 : 1, NULL, NULL,
                 NULL, -1, subst_mod_is_codon_model(mod->subst_mod));
...
#pragma omp parallel private(i, j, k, pass, col_offset, nodeidx, rcat, n,      \
  ...
  {
    double **inside_joint = (double **)smalloc(nstates * sizeof(double *));
    for (j = 0; j < nstates; j++)
      inside_joint[j] =
          (double *)smalloc((mod->tree->nnodes + 1) * sizeof(double));
    double **outside_joint = (double **)smalloc(nstates * sizeof(double *));
    for (j = 0; j < nstates; j++)
      outside_joint[j] =
          (double *)smalloc((mod->tree->nnodes + 1) * sizeof(double));
    double **inside_marginal = NULL;
    double **outside_marginal = NULL;
    if (mod->order > 0) {
      inside_marginal = (double **)smalloc(nstates * sizeof(double *));
      for (j = 0; j < nstates; j++)
        inside_marginal[j] =
...
// parallelize tuples equally across threads and sum the results
#pragma omp for schedule(static) reduction(+ : retval)
    for (tupleidx = 0; tupleidx < msa->ss->ntuples; tupleidx++) {
              ...
              if (n->lchild == NULL) {
                ...
                for (col_offset = -1 * mod->order; col_offset <= 0;
                  ...
                  if (pass == 0 || col_offset < 0) {
                    char thischar =
                        ss_get_char_tuple(msa, tupleidx, thisseq, col_offset);
                    observed_state =
                        mod->rate_matrix->inv_states[(int)thischar];
                    if (observed_state < 0)
                ...
                for (i = 0; i < nstates; i++) {
                  if (mod->order == 0) /* handle 0th order model as special
                    ...
                    int total_match = 1;
                    /* figure out the "projection" of state i in the
                       dimension of each position, and see whether there is a
                       corresponding partial match. */
                    /* NOTE: mod->order is approx equal to log nstates
                       (prob no more than 2) */
                    for (col_offset = -1 * mod->order;
                         col_offset <= 0 && total_match; col_offset++) {
                      int projection =
                          (i / int_pow(alph_size, -1 * col_offset)) % alph_size;
            ...
            if (post != NULL && pass == 0) {
              ...
              for (nodeidx = 0; nodeidx < lst_size(traversal); nodeidx++) {
                ...
                if (n->parent == NULL) { /* base case */
                  ...
                  TreeNode *sibling =
                      (n == n->parent->lchild ? n->parent->rchild
                                              : n->parent->lchild);
                  ...
                  for (j = 0; j < nstates; j++) { /* parent state */
                    ...
                    for (k = 0; k < nstates; k++) { /* sibling state */
                      tmp[j] += pLbar[j][n->parent->id] * pL[k][sibling->id] *
      ...
      if (post != NULL) {
        if (skip_fels)
          die("ERROR: tl_compute_log_likelihood: skip_fels should be 0 but is "
              "%i\n",
              skip_fels);
        for (rcat = 0; rcat < mod->nratecats; rcat++) {
          ...
          // atomic update of cross-thread shared variables
          if (post->rcat_expected_nsites != NULL) {
#pragma omp atomic
            post->rcat_expected_nsites[rcat] +=
                rcat_post_prob * (cat >= 0 ? msa->ss->cat_counts[cat][tupleidx]
          ...
          if (post->expected_nsubst_tot != NULL) {
            for (nodeidx = 0; nodeidx < mod->tree->nnodes; nodeidx++) {
              ...
              for (i = 0; i < nstates; i++)
                for (j = 0; j < nstates; j++) {
                  double add = subst_probs[rcat][i][j][n->id] *
                               (cat >= 0 ? msa->ss->cat_counts[cat][tupleidx]
                                         : msa->ss->counts[tupleidx]) *
                               rcat_post_prob;
#pragma omp atomic
                  post->expected_nsubst_tot[rcat][i][j][n->id] += add;
                }
            }
          }
    ...
    for (j = 0; j < nstates; j++) {
      ...
      if (mod->order > 0)
        sfree(inside_marginal[j]);
      if (mod->order > 0 && post != NULL)
        sfree(outside_marginal[j]);
    }
    sfree(inside_joint);
    sfree(outside_joint);
    if (mod->order > 0)
      sfree(inside_marginal);
    if (mod->order > 0 && post != NULL)
      sfree(outside_marginal);
    if (post != NULL) {
      for (rcat = 0; rcat < mod->nratecats; rcat++) {
        for (j = 0; j < nstates; j++) {
          for (k = 0; k < nstates; k++)
  ...
  if (col_scores != NULL) {
    ...
    if (tuple_scores == NULL)
      sfree(curr_tuple_scores);
  }
...
void tl_compute_log_likelihood_weight_matrix(TreeModel *mod, MSA *msa,
  ...
  Vector *margfreqs = get_marginal_eq_freqs(mod->rate_matrix->states,
                                            mod->order + 1, mod->backgd_freqs);
  ...
  if (mod->tree != NULL)
    die("ERROR tl_compute_log_likelihood_weight_matrix: mod->tree should be "
        "NULL\n");
  if (msa->ss == NULL && msa->seqs == NULL)
    die("ERROR tl_compute_log_likelihood_weight_matrix: mod->ss and mod->seqs "
        "are both NULL\n");

  if (cat >= 0) {
    if (col_by_col) {
      if (msa->categories == NULL)
        die("ERROR tl_compute_log_likelihood_weight_matrix: msa->categories is "
            "NULL\n");
    }
    /* if using categories and col-by-col
       scoring, then must have col-by-col
      ...
      die("ERROR tl_compute_log_likelihood_weight_matrix: msa->ss->cat_counts "
          "is NULL\n");
  ...
  retval = 0;
  for (idx = 0; idx < (col_by_col ? msa->length : msa->ss->ntuples); idx++) {
    ...
    if (msa->ss == NULL)
      ...
      tupleidx = msa->ss->tuple_idx[col];
    else
      tupleidx = idx; /* NOTE: tupleidx will be defined
    ...
    if (cat < 0 || (col_by_col && msa->categories[col] == cat) ||
      ...
      for (seq = 0; seq < msa->nseqs; seq++) {
        ...
        if (!mod->allow_gaps && msa->inv_alphabet[(int)tuple[mod->order]] < 0 &&
          ...
          for (i = 0; i < alph_size; i++) {
            ...
            tmp_prob =
                vec_get(margfreqs,
                        tuple_index_missing_data(tuple, msa->inv_alphabet,
        ...
        if (mod->use_conditionals && mod->order > 0) {
          ...
          col_val -= log2(vec_get(
              margfreqs, tuple_index_missing_data(tuple, msa->inv_alphabet,
                                                  msa->is_missing, alph_size)));
        }
      }
    }
...
TreePosteriors *tl_new_tree_posteriors(TreeModel *mod, MSA *msa, int do_bases,
  ...
  if (do_substs) {
    tp->subst_probs =
        (double *****)smalloc(mod->nratecats * sizeof(double ****));
    for (r = 0; r < mod->nratecats; r++) {
      checkInterrupt();
      tp->subst_probs[r] = (double ****)smalloc(nstates * sizeof(double ***));
      for (i = 0; i < nstates; i++) {
        tp->subst_probs[r][i] =
            (double ***)smalloc(nstates * sizeof(double **));
        for (j = 0; j < nstates; j++) {
          tp->subst_probs[r][i][j] =
              (double **)smalloc(nnodes * sizeof(double *));
          for (k = 0; k < nnodes; k++)
            if (k != mod->tree->id) /* don't need one for the root */
              tp->subst_probs[r][i][j][k] =
                  (double *)smalloc(ntuples * sizeof(double));
            else
  ...
  if (do_expected_nsubst) {
    ...
    tp->expected_nsubst = NULL;

  if (do_expected_nsubst_tot) {
    tp->expected_nsubst_tot =
        (double ****)smalloc(mod->nratecats * sizeof(double ***));
    for (r = 0; r < mod->nratecats; r++) {
      tp->expected_nsubst_tot[r] =
          (double ***)smalloc(nstates * sizeof(double **));
      for (i = 0; i < nstates; i++) {
        tp->expected_nsubst_tot[r][i] =
            (double **)smalloc(nstates * sizeof(double *));
        for (j = 0; j < nstates; j++)
          tp->expected_nsubst_tot[r][i][j] =
              (double *)smalloc(nnodes * sizeof(double));
      }
    }
  } else
    tp->expected_nsubst_tot = NULL;

  if (do_expected_nsubst_col) {
    tp->expected_nsubst_col =
        (double *****)smalloc(mod->nratecats * sizeof(double ****));
    for (r = 0; r < mod->nratecats; r++) {
      tp->expected_nsubst_col[r] =
          (double ****)smalloc(nnodes * sizeof(double ***));
      for (i = 0; i < nnodes; i++) {
        tp->expected_nsubst_col[r][i] =
            (double ***)smalloc(ntuples * sizeof(double **));
        for (j = 0; j < ntuples; j++) {
          tp->expected_nsubst_col[r][i][j] =
              (double **)smalloc(nstates * sizeof(double *));
          for (k = 0; k < nstates; k++)
            tp->expected_nsubst_col[r][i][j][k] =
                (double *)smalloc(nstates * sizeof(double));
        }
      }
    }
  } else
    tp->expected_nsubst_col = NULL;
  ...
  if (do_rate_cats_exp)
    tp->rcat_expected_nsites =
        (double *)smalloc(mod->nratecats * sizeof(double));
  else
    tp->rcat_expected_nsites = NULL;
  ...
  for (i = 0; i < ntuples; i++) {
    ...
    for (k = 0; k < (1 << tuple_size); k++) {
      ...
      vec_set(retval, newtuple,
              vec_get(retval, newtuple) + vec_get(eq_freqs, i));
    }
  }
  return retval;
}
