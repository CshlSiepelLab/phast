...
void col_scale_derivs_subst_complex(ColFitData *d)
{
  ...
  {
    for (nid = 1; nid < d->mod->tree->nnodes; nid++)
    { /* skip root */

      double t = ((TreeNode *)lst_get_ptr(d->mod->tree->nodes, nid))->dparent;
      double l1 = d->mod->scale;
      double l2 = (d->stype == SUBTREE && d->mod->in_subtree[nid] ? d->mod->scale_sub : 1);
  ...
  *first_deriv = 0;
  ...
  for (rcat = 0; rcat < d->mod->nratecats; rcat++)
  {
    for (nodeidx = 0; nodeidx < lst_size(traversal); nodeidx++)
    {
      ...
      {
        /* leaf: base case of recursion */
        int state = d->mod->rate_matrix->inv_states[(int)ss_get_char_tuple(d->msa, d->tupleidx,
                     ...
                     double *tuple_teststats)
{
  ...
  for (i = 0; i < msa->ss->ntuples; i++)
  {
    ...
    /* store scales and log likelihood ratios if necessary */
    if (tuple_derivs != NULL)
      tuple_derivs[i] = first_deriv;
    if (tuple_teststats != NULL)
      tuple_teststats[i] = teststat;
  }
                         ...
                         double *tuple_teststats, FILE *logf)
{
  ...
  for (i = 0; i < msa->ss->ntuples; i++)
  {
    ...
    if (tuple_null_scales != NULL)
      tuple_null_scales[i] = d->params->data[0];
    if (tuple_derivs != NULL)
      tuple_derivs[i] = grad->data[0];
    if (tuple_sub_derivs != NULL)
      tuple_sub_derivs[i] = grad->data[1];
    if (tuple_teststats != NULL)
      tuple_teststats[i] = teststat;
  }
              ...
              double *tuple_nspec, FILE *logf)
{
  ...
  for (i = 0; i < msa->ss->ntuples; i++)
  {
    ...
    if (tuple_nspec != NULL)
      tuple_nspec[i] = (double)nspec;
    if (tuple_nneut != NULL)
      tuple_nneut[i] = nneut;
    if (tuple_nobs != NULL)
      tuple_nobs[i] = scale * nneut;
    if (tuple_nrejected != NULL)
    {
      ...
      if (mode == ACC)
        ...
        tuple_nrejected[i] = fabs(tuple_nrejected[i]);
    }
  }
  col_free_fit_data(d);
  sfree(has_data);
                               ...
                               int *has_data, int *nspec)
{
  ...
  *nspec = 0;
  for (i = 0; i < lst_size(traversal); i++)
  {
    ...
    { /* leaf */
      if (mod->rate_matrix->inv_states[(int)ss_get_char_tuple(msa, tupleidx,
...
int col_has_data(TreeModel *mod, MSA *msa, int tupleidx)
{
  ...
  {
    int state = mod->rate_matrix->inv_states[(int)ss_get_char_tuple(msa, tupleidx, i, 0)];
    if (state >= 0)
      nbases++;
  }
  return (nbases >= 2);
                     ...
                     List *outside)
{
  ...
  if (inside == NULL && outside == NULL)
  {
    for (i = 0; i < mod->tree->nnodes; i++)
    {
      ...
      if (n->lchild == NULL)
      {
        state = mod->rate_matrix->inv_states[(int)ss_get_char_tuple(msa, tupleidx,
                                                                    mod->msa_seq_idx[n->id], 0)];
        if (state >= 0)
          nbases++;
        if (nbases == 2)
