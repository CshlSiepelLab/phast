...
/* fit a tree model using EM */
int tm_fit_em_mess(TreeModel *mod, MSA *msa, Vector *params, int cat, 
              opt_precision_type precision, FILE *logf) {
  double ll, improvement;
  Vector *lower_bounds, *upper_bounds;
  int retval = 0, it, i, home_stretch = 0, nratecats, empirical_rates;
  double lastll = NEGINFTY, alpha, *rKOrig=NULL, *freqKOrig=NULL, 
    branchlen_scale;
  struct timeval start_time, end_time, post_prob_start, post_prob_end;
  TreeModel *proj_mod = NULL;
  char tmp_mod_fname[STR_SHORT_LEN];
  ...
  /* in the case of rate variation, start by ignoring then reinstate
     when close to convergence */
  nratecats = mod->nratecats;
  empirical_rates = mod->empirical_rates;
  if (nratecats > 1) {
    alpha = mod->alpha;  /* code to ignore rate variation temporarily */
    rKOrig = smalloc(nratecats * sizeof(double));
    freqKOrig = smalloc(nratecats * sizeof(double));
    for (i=0; i<nratecats; i++) {
      rKOrig[i] = mod->rK[i];
      freqKOrig[i] = mod->freqK[i];
    }
    mod->freqK[0] = mod->rK[0] = 1;

    tm_reinit(mod, mod->subst_mod, 1, 0, NULL, NULL);

    vec_free(params);
    params = tm_params_new_init_from_model(mod);
  }
  ...
  for (it = 1; ; it++) {
    ...
    if (mod->nratecats != nratecats && 
      ...
      if (logf != NULL) fprintf(logf, "Introducing rate variation.\n");
      vec_free(params);
      tm_reinit(mod, mod->subst_mod, nratecats, alpha, NULL, NULL);
      mm_set_eigentype(mod->rate_matrix, COMPLEX_NUM);
	...
	for (i = 0; i < mod->backgd_freqs->size; i++)
      ...
      mod->empirical_rates = empirical_rates;
      for (i=0; i<nratecats; i++) {
	mod->rK[i] = rKOrig[i];
	mod->freqK[i] = freqKOrig[i];
      }      
      params = tm_params_new_init_from_model(mod);
      numpar=0;
      for (i=0; i<params->size; i++)
	if (mod->param_map[i] >=0) numpar++;
      vec_free(optpar);
      optpar = vec_new(numpar);
      for (i=0; i<params->size;i++)
	if (mod->param_map[i] >=0)
  ...
  mat_free(H);
  vec_free(optpar);
  if (rKOrig != NULL) free(rKOrig);
  if (freqKOrig != NULL) free(freqKOrig);
