...
void base64_encode_block( unsigned char in[3], unsigned char out[4], int len )
{
  out[0] = b64[ in[0] >> 2 ];
...
char *base64_encode( char *inStr )
{
  ...
  origLen = strlen(inStr);
  finalOut = (char*)malloc(sizeof(char) * (2 * origLen));  

  for(ch=0; ch<origLen; ch=ch+3) {
    len = 0;
    for( i = 0; i < 3; i++ ) {
      in[i] = (unsigned char)inStr[ch+i];
      if((ch+i) < origLen) {
        len++;
      }
      else {
    ...
    if( len ) {
      ...
      finalOut[((ch/3)*4)] = out[0];
      finalOut[((ch/3)*4)+1] = out[1];
      finalOut[((ch/3)*4)+2] = out[2];
      finalOut[((ch/3)*4)+3] = out[3];
      blocksout++;
    }
  }
  finalOut[(ch/3)*4] = '\0';
