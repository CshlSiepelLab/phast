...
int main(int argc, char *argv[]) {
  ...
  while ((c = getopt_long(argc, argv, "L:n:i:d:qht:s:k:Ep:M:r", long_opts, &opt_idx)) != -1) {
    switch (c) {
    case 'L':
  ...
  if (!parameteric) {
    ...
    if (msa->ss == NULL)
      ss_from_msas(msa, tm_order(subst_mod) + 1, 0, NULL, NULL, NULL, -1);
  ...
  for (i = 0; i < nreps; i++) {
    ...
    if (parameteric) 
      ...
      mn_draw(nsites, p, msa->ss->ntuples, tmpcounts);
                                /* here we simply redraw numbers of
                                   tuples from mulinomial distribution
    ...
    if (init_mod == NULL) 
      ...
      thismod = init_mod;       /* FIXME: make copy -- will be
                                   inherited from one rep to the
                                   next */
    ...
    if (dump_fname_root != NULL) {
      char fname[STR_MED_LEN];
      sprintf(fname, "%s.%d.mod", dump_fname_root, i+1);
      F = fopen_fname(fname, "w+");
      tm_print(F, thismod);
      fclose(F);
    }

    /* set up record of all parameter estimates; easiest to do this
       way because number of parameters not always known ahead of
       time */
    if (nparams <= 0) {
      nparams = params->size;
    ...
    if (init_mod == NULL) tm_free(thismod);
  }

  /* finally, compute and print stats for params */
